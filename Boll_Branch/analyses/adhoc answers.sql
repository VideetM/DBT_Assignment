...
-- Note: I avoided using rank function wherever I can, I genreally observed differing values hence didn't find a need. 
-- 1st adhoc query
...
-- 3rd adhoc query
SELECT extract(DATE FROM order_created_at) AS highest_avg_date,
	avg(quantity) AS highest_avg_unit_order
FROM `data - recruiting.ae_data_challenge_v1_vmalpe.orders_report`
GROUP BY 1
ORDER BY 2 DESC limit 1

--2022-08-03 with 1.374 avg unit per order. .

-- 4th adhoc query
WITH plush_order_session AS (
